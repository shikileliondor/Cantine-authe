...
<body class="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 sm:flex sm:items-center sm:justify-center">
    <main class="mx-auto w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-900/95 p-6 shadow-2xl shadow-slate-950/60 sm:p-8">
        ...
        <form method="POST" action="{{ route('pin.authenticate') }}" class="space-y-5" id="pin-form">
            @if (session('status'))
                <p class="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200">{{ session('status') }}</p>
            @endif
            @csrf
            ...
            <div class="space-y-3 rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-300">
                <p>PIN oublié ? Créez/réinitialisez votre PIN avec le code administrateur.</p>
                <a href="{{ route('pin.setup') }}" class="inline-flex rounded-lg border border-indigo-400/60 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-indigo-200 transition hover:bg-indigo-500/20">Créer ou réinitialiser mon PIN</a>
            </div>
        </form>
    </main>
